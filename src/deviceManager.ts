...
.vkb-key-arrow{flex:0.8;}\`;
  ...
  function processKey(key){
    ...
    switch(key){
      case '▼': hideKeyboard(); break;
      case '⇧': isShifted = !isShifted; currentLayout = isShifted ? 'shift' : 'default'; renderKeyboard(); break;
      case '?123': currentLayout = 'symbols'; isShifted = false; renderKeyboard(); break;
      ...
      case '⏎':
        if(activeInput.tagName === 'TEXTAREA'){
          ...
          const ev = {key:'Enter',code:'Enter',keyCode:13,which:13,bubbles:true,composed:true,cancelable:true};
      ...
      default:
        ...
        if(isShifted){ isShifted = false; currentLayout = 'default'; renderKeyboard(); }
        break;
    }
  ...
  function hideKeyboard(){
    console.log('[VKB] hideKeyboard triggered.');
    window.__vkbClosingShield = Date.now();
    if(keyboardContainer) keyboardContainer.classList.remove('vkb-visible');
  ...
  interactionEvents.forEach(ev => {
    document.addEventListener(ev, function(e){
      ...
      if(ev === 'pointerdown'){
        const path = e.composedPath ? e.composedPath() : [e.target];
        if(!resolveInputFromPath(path)) hideKeyboard();
      }
    }, true);
  });
